...
void control(){
  ...
  if((inicio_prueba == "NO" || inicioPruebaEntero == 0) && (setTiempoApagado !=0 && setTiempoPrendido !=0 )) {
    ...
    tiempo_prueba = ((tiempo_actual - tiempo_inicio - tiempoPausadoAcumulado)) + tiempo_pruebaReinicio;
    
    estado_prueba = "Sistema funcionando";

    tiempoPruebaA = tiempo_prueba - tiempoPruebaB;
...
void mandar_datos(){
  uint64_t now = millis();
    ...
    if(now - messageTimestamp > 1000) {
        ...
        param1["tiempo_transcurrido"] = tiempo_prueba / 1000;
